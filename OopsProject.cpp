...
#include <vector>
#include <string>
#include <conio.h>  //? For _getch() to hide password on Windows
...
class User {
public:
    string name;       //*basic user detail
    int age;
    virtual void showDetails() = 0; // Pure virtual function
};

//  Derived Class from user (inherit)
class Patient : public User {
public:                        //extra details
    string bloodGroup;
    int quantity;
    ...
    void showDetails() override {   //patient info
...
class BloodBank {
    map<string, int> bloodInventory;  //?to show availability
    vector<Donor> donors;  //list of donars

    string adminUsername = "Somaresa";   //for admin login
    string adminPassword = "12345";

public:
    BloodBank() {   //?initialised blood stock
        bloodInventory["A+"] = 5;
        bloodInventory["A-"] = 3;
        bloodInventory["B+"] = 4;
        bloodInventory["B-"] = 2;
    ...
    void registerDonor() {   //?member fn collect and store detail
        Donor d;
        cout << "\n--- Donor Registration ---\n";
        cin.ignore();
    ...
    void donateBlood() {
        ...
        bool found = false; //if donar exist
        string group;
    ...
    bool adminLogin() {
        ...
        while (true) {
            ch = _getch();  // Read without displaying
            if (ch == 13) break;             
            else if (ch == 8) {
                if (!password.empty()) {
                    cout << "\b \b";
